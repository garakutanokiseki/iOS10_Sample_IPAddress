...
class NetworkInfo {
    ...
    static func LocalWifiIPv4() -> String {
        ...
        guard let i = list.index(where: { $0.device == Device.LocalWifi.rawValue && $0.version == .iPv4 }) else { return "" }
        return list[i].IP
    }

    static func WanIPv4() -> String {
        guard let list = IPAddressList() else { return "" }
        guard let i = list.index(where: { $0.device == Device.Wan.rawValue && $0.version == .iPv4 }) else { return "" }
        return list[i].IP
    }
}
